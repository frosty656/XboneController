...
void Gopher::loop() {
	...
	if (_xboxClickIsDown[CONFIG_OPEN_KEYBOARD]) {
		...
		std::string filedirectory = workingdir().c_str();

		filedirectory.append("keyboard.exe");

		ShellExecute(0, "open", filedirectory.c_str(), NULL, NULL, SW_SHOWNORMAL);

		switch (keyboard) {
		case 1:
			ShowWindow(FindWindow(NULL, "On-Screen Keyboard"), SW_SHOWNORMAL);
			keyboard = 2;
			printf("Showing Keyboard...\n");
			break;
		case 2:
			ShowWindow(FindWindow(NULL, "On-Screen Keyboard"), SW_HIDE);
			keyboard = 1;
